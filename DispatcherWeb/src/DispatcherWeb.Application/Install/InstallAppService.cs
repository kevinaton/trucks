...
namespace DispatcherWeb.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : DispatcherWebAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<DispatcherWebDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{DispatcherWebConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{DispatcherWebConsts.ConnectionStringName}", constring);
        }
        ...
        private async Task SetSmtpSettings(Configuration.Dto.EmailSettingsEditDto input)
        {
