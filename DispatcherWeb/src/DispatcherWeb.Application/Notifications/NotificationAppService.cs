...
namespace DispatcherWeb.Notifications
{
    ...
    public class NotificationAppService : DispatcherWebAppServiceBase, INotificationAppService
    {
        ...
        {
            var notifications = await _userNotificationManager.GetUserNotificationsAsync(AbpSession.ToUserIdentifier(), UserNotificationState.Unread);
            notifications = notifications.Where(x => x.Notification.NotificationName == AppNotificationNames.PriorityNotification).ToList();
            return new GetNotificationsOutput(notifications.Count, notifications.Count, notifications);
        }
