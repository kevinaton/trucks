...
(function ($) {
    ...
    app.ModalManager = (function () {
        ...
        return function (options) {
            ...
            function _initAndShowModal(openResultDeferred) {
            ...
            var _open = function (args, getResultCallback) {
                ...
                _createContainer(_modalId, _options.modalSize)
                    .find('.modal-content')
                    .load(options.viewUrl, _args, function (response, status, xhr) {
                        if (status === "error") {
                            ...
                            if (responseObject && responseObject.userFriendlyException) {
                                ...
                                messageDeferred = abp.message.warn(e.details, e.message);
                            } else {
                                messageDeferred = abp.message.warn(abp.localization.abpWeb('InternalServerError'));
                            }
                            messageDeferred.done(function () {
                                openResultDeferred.rejectWith(_publicApi, [{
                                    loadResponse: response,
                                    loadResponseObject: responseObject,
                                    loadStatus: status,
                                    loadXhr: xhr
                                }]);
                            });
                        ...
                        if (options.scriptUrl && _loadedScripts.indexOf(options.scriptUrl) < 0) {
                            $.getScript(options.scriptUrl)
                                .done(function (script, textStatus) {
                                    _loadedScripts.push(options.scriptUrl);
                                    _initAndShowModal(openResultDeferred);
                                })
                                .fail(function (jqxhr, textStatus, exception) {
                                    abp.message.warn(abp.localization.abpWeb('InternalServerError')).done(function () {
                                        openResultDeferred.rejectWith(_publicApi, [{
                                            scriptLoadXhr: xhr,
                                            scriptLoadStatus: textStatus,
                                            scriptLoadErrorThrown: exception
                                        }]);
                                    });
                                });
                        } else {
