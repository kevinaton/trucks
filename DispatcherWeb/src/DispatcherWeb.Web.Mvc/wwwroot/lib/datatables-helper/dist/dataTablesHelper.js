...
(function ($) {
    ...
    abp.helperConfiguration.dataTables.getNewCheckboxContainer = function (options) {
        options = options || {};
        var container, input;
        container = $('<label class="m-checkbox cell-centered-checkbox">').append(input = $('<input type="checkbox" class="minimal">')).append($('<span>'));
        if (options.disabled) {
    ...
    abp.helper.dataTables = function () {
        ...
        function renderUtcDateTime(value, emptyValue) {
            if (value) {
                var localValue = parseUtcDateTime(value);
        ...
        function formatNumber(value) {
            return ('' + value).replace(/(\d)(?=(?:\d{3})+(?:\.|$))|(\.\d\d?)\d*$/g, function (m, s1, s2) {
                return s2 || (s1 + ',');
            });
        }
            ...
            function decimal(editorOptions, gridOptions) {
                ...
                return function (cell, cellData, rowData, rowIndex, colIndex) {
                    ...
                    editor.focusout(function () {
                        ...
                        if (editorOptions.maxValue !== undefined && parseFloat(newValue) > editorOptions.maxValue) {
                            abp.message.error('Please enter a valid number less than ' + editorOptions.maxValue + '!');
                            editorClosedCallback();
                            return;
                        }
                        if (editorOptions.minValue !== undefined && parseFloat(newValue) < editorOptions.minValue) {
                        ...
                        newValue = abp.utils.round(parseFloat(newValue));
            ...
            class DelayedSaveQueue {
                ...
                add(updatedRow, updatedEditor) {
                    ...
                    this.timer = setTimeout(() => {
                        ...
                        Promise.resolve(this.options.saveCallback(queue, queuedEditors)).then(function () {
                            if (setBusyOnSave) {
            ...
            function checkbox(editorOptions, gridOptions, columnIndex) {
                ...
                if (editorOptions.addHeaderCheckbox) {
                    ...
                    gridOptions.headerCallback = function (thead, data, start, end, display) {
                        ...
                        let headerCheckbox = headerCheckboxContainer.input;
                ...
                return function (cell, cellData, rowData, rowIndex, colIndex) {
                    ...
                    let editor = checkboxContainer.input;
        ...
        return {
            ...
            parseDateTimeAsUtc: parseDateTimeAsUtc,
    ...
    function getSelectionColumnObject(selectionColumnOptions) {
        ...
        var _grid = null;
        var _options = null;
        var selectionChangedCallbacks = []; //expected an array of "function (selectedRowsCount)" handlers
    ...
    jQuery.fn.DataTableInit = function (userOptions) {
        ...
        let afterInitCallbacks = abp.helperConfiguration.dataTables.afterInit;
...
//# sourceMappingURL=dataTablesHelper.js.map